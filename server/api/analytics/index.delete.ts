...
export default defineEventHandler(async (event) => {
  ...
  await db.$transaction(async (tx) => {
    if (!query.type || query.type === "pageView") {
      const result = await tx.pageView.deleteMany({ where: { userId: user.id, ...(hasDateFilter && { createdAt: dateFilter }) } })
    ...
    if (!query.type || query.type === "linkClick") {
      ...
      if (userLinks.length > 0) {
        const result = await tx.linkClick.deleteMany({ where: { userLinkId: { in: userLinks.map(link => link.id) }, ...(hasDateFilter && { createdAt: dateFilter }) } })
        deletedCount += result.count
    ...
    if (!query.type || query.type === "iconClick") {
      ...
      if (userIcons.length > 0) {
        const result = await tx.iconClick.deleteMany({ where: { userIconId: { in: userIcons.map(icon => icon.id) }, ...(hasDateFilter && { createdAt: dateFilter }) } })
        deletedCount += result.count
