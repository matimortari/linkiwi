<template>
  ...
  <div v-if="user" class="mx-auto flex justify-center select-none">
    ...
    <transition name="slide">
      <div v-if="isPreviewOpen" class="fixed top-0 left-0 z-40 size-full overflow-y-auto pb-12 md:hidden" :style="backgroundStyle">
    ...
    >
      <div class="absolute top-2 left-1/2 z-30 h-1.5 w-24 -translate-x-1/2 rounded-full bg-black" />
      <div class="absolute top-2 z-30 w-full px-6">
        <div class="flex justify-end gap-2 text-[#eeeeee]">
          <icon name="mdi:signal" size="15" />
          <icon name="mdi:wifi" size="15" />
        </div>
      </div>

      <div v-if="user.banner?.url" class="absolute top-0 left-0 h-32 w-full">
        <img :src="user.banner.url" alt="Profile Banner" class="size-full object-cover">
      </div>

      <div class="flex flex-col items-center justify-start gap-4 p-4 text-center" :class="user.banner?.url ? 'relative z-10 pt-20' : 'pt-12'">
        <div class="flex flex-col items-center gap-2">
