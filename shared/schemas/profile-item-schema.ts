...
const iconPayload = z.object({
  url: z.url("Invalid URL").refine(url => url.startsWith("http://") || url.startsWith("https://"), { message: "URL must start with http:// or https://" }),
  platform: z.string().min(1).max(50),
  logo: z.string().min(1).max(100),
})

const widgetPayload = z.object({
  type: z.enum(["GITHUB", "YOUTUBE", "SPOTIFY"]),
  handle: z.string().min(1, "Handle is required").max(300).transform(val => val.trim()),
})
